...
Unpack the zip in a convenient location.

Open a terminal, and navigate to the directory.

add the mark sidebar (this takes maybe 10sec)
```gradex-overlay.exe edited5.pdf mark```

Then open edited5-mark.pdf to check red artwork is present on right hand side of the pages.

Repeat for the other stages in the process ...

```
gradex-overlay.exe edited5-mark.pdf moderate-inactive
gradex-overlay.exe edited5-mark.pdf moderate-active
gradex-overlay.exe edited5-mark-moderate-inactive.pdf check
gradex-overlay.exe edited5-mark-moderate-active.pdf check
```


## Performance

The performance is coming in at around just over a second per page, mostly time spent rendering the PDF to image, to save the annotations. 

```
./gradex-overlay test-script-mark-moderate-active.pdf check  5.62s user 0.36s system 106% cpu 5.618 total
```
